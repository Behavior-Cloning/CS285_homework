...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        D = self.forward(ptu.from_numpy(observation))
    ...
    def forward(self, observation: torch.FloatTensor):
        if self.discrete:
            ...
            # tril_Cov_mat = torch.diag(self.logstd.exp())
            tril_Cov_mat = torch.diag(torch.exp(self.logstd))
            mu = self.mean_net(observation)
            return torch.distributions.MultivariateNormal(mu, scale_tril=tril_Cov_mat)
