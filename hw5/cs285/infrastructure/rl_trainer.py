...
class RL_Trainer(object):
    ...
    def __init__(self, params):
        ...
        if 'non_atari_colab_env' in self.params and self.params['video_log_freq'] > 0:
            self.env = wrappers.Monitor(self.env, os.path.join(self.params['logdir'], "gym"),
                                        write_upon_reset=True)  # , force=True)
            self.eval_env = wrappers.Monitor(self.eval_env, os.path.join(self.params['logdir'], "gym"),
    ...
    def collect_training_trajectories(self, itr, load_initial_expertdata, collect_policy, num_transitions_to_sample,
        ...
        if load_initial_expertdata is not None and itr == 0:
            with open(load_initial_expertdata, 'rb') as f:
                loaded_experdata = pickle.loads(f.read())
            return loaded_experdata, 0, None
