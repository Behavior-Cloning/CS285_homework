...
class MPCPolicy(BasePolicy):
    ...
    def calculate_sum_of_rewards(self, obs, candidate_action_sequences, model):
        ...
        observations = np.array([obs] * num_sequences)
        for t in range(horizon):
            actions = candidate_action_sequences[:, t, :]
            assert actions.shape == (num_sequences, self.ac_dim)
            next_observations = model.get_prediction(observations, actions, self.data_statistics)
            rewards,_ = self.env.get_reward(next_observations,actions)
            #print(rewards)
            assert rewards.shape == (num_sequences,)
